...
<div class="max-w-7xl mx-auto px-4 py-10">
    ...
    @php
        ...
        $color = $statusColors[$order->status] ?? 'bg-zinc-900 text-gray-300 border-zinc-700';
    @endphp
    <div class="bg-zinc-900 rounded-2xl shadow mb-6 overflow-hidden border-l-4 {{ str_contains($color, 'yellow') ? 'border-yellow-400' : (str_contains($color, 'blue') ? 'border-blue-400' : (str_contains($color, 'orange') ? 'border-red-400' : 'border-green-400')) }}">
        <div class="p-6">
            <div class="flex items-start justify-between flex-wrap gap-4">
                ...
                <form method="POST" action="{{ route('kitchen.update-status', $order) }}" class="flex items-center gap-2">
                    @csrf @method('PATCH')
                    <select name="status" class="border border-zinc-700 rounded-lg px-3 py-2 bg-zinc-950 text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-400">
